...
router.get('/all', async (req, res) => {
  try {
    const usuarios = await user.getCategories();
...
router.put('/update/:id', async (req, res)=>{
  try {
    const {id} = req.params;
    if(!(/^\d+$/.test(id))) {
      return res.status(400).json({error:'El id debe ser un dígito válido.'});
    }
    const {email,
        nombre,
        avatar,
    ...
    const updateResult = await user.updateUsuario({id:parseInt(id),email,nombre,avatar,password,estado});
